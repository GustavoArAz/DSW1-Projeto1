...
// Reference to the 'lfgForm' node in the database
const lfgFormRef = ref(database, 'lfgForm');

// Query the last 5 entries from the 'lfgForm' node
const recentPostsQuery = query(lfgFormRef, limitToLast(5));
...
onValue(recentPostsQuery, (snapshot) => {
    ...
    if (data) {
        ...
        Object.entries(data).forEach(([key, item]) => {
            ...
            titleLink.href = "Tela LFG.html";  // Adjust the link as needed
            titleLink.textContent = item.title;
            titleCell.appendChild(titleLink);

            const nplayersCell = document.createElement('th');
            nplayersCell.textContent = item.nplayers;
...
function editRow(key, row, item) {
    ...
    const nplayersCell = document.createElement('th');
    const nplayersInput = document.createElement('input');
    nplayersInput.type = 'text';
    nplayersInput.value = item.nplayers;
    nplayersCell.appendChild(nplayersInput);
    ...
    saveButton.addEventListener('click', () => saveEdit(key, titleInput.value, nplayersInput.value, dscInput.value));
...
function saveEdit(key, title, nplayers, dsc) {
    const updates = {};
    updates[`lfgForm/${key}`] = { title, nplayers, dsc };
...
function deleteItem(key) {
    if (confirm('Are you sure you want to delete this item?')) {
        remove(ref(database, `lfgForm/${key}`))
            .then(() => {
